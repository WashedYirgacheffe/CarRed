                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className={clsx(
                                        ...
                                        (advisor.avatar.startsWith('http') || advisor.avatar.startsWith('local-file'))
                        ...
                        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className={clsx(
                                    ...
                                    (selectedAdvisor.avatar.startsWith('http') || selectedAdvisor.avatar.startsWith('local-file'))
                                        ? "bg-transparent border border-border"
                                        : AVATAR_COLORS[parseInt(selectedAdvisor.id.slice(-1), 16) % AVATAR_COLORS.length]
                                )}>
                                    {(selectedAdvisor.avatar.startsWith('http') || selectedAdvisor.avatar.startsWith('local-file')) ? (
                                        <img src={selectedAdvisor.avatar} alt={selectedAdvisor.name} className="w-full h-full object-cover" />
                                    ) : (
...
function VideoManagement({ advisorId }: { advisorId: string }) {
    ...
    const [youtubeChannel, setYoutubeChannel] = useState<{ url: string; lastRefreshed?: string } | null>(null);
    ...
    const handleRetryAll = async () => {
        const failedVideos = videos.filter(v => v.status === 'failed' && v.retryCount < 5);
        for (let i = 0; i < failedVideos.length; i++) {
            if (i > 0) await new Promise(r => setTimeout(r, 3000)); // 3s delay to avoid 429
            await handleDownload(failedVideos[i].id);
        }
    };
    ...
    // Don't render if not a YouTube advisor
...
function AdvisorModal({
    ...
    const handleSubmit = () => {
        ...
        onSave({ name, avatar, personality, systemPrompt }, ytParams);
    };
    ...
    const handleSelectAvatar = async () => {
        try {
            ...
            if (filePath) {
                // For preview, we can use the local path directly (renderer can read local files via local-file protocol if handled,
                // or we rely on the fact that we are in Electron.
                // However, standard img src won't read local path directly due to security.
                // We might need to convert to local-file:// for preview if we added that protocol support.
                // Based on main.ts, we have 'local-file' protocol registered.
                const previewUrl = `local-file://${filePath}`;
                ...
                <div className="px-6 py-4 space-y-4 overflow-auto flex-1">
                                ...
                                <>
                                    ...
                                    {youtubeInfo && (
                                        <div className="flex items-start gap-3 bg-surface-primary p-3 rounded-lg border border-border">
                                            <img src={youtubeInfo.avatarUrl} alt="" className="w-10 h-10 rounded-full object-cover shrink-0" />
                    ...
                    <div>
                        ...
                        <div className="flex gap-2 flex-wrap items-center">
                            ...
                            {(avatar.startsWith('http') || avatar.startsWith('local-file') || avatar.startsWith('data:')) && (
