...
export function CreativeChat({ activeFile }: { activeFile?: { path: string; content: string } }) {
    ...
    const getRoomAdvisors = (room: ChatRoom) => {
        return advisors.filter(a => room.advisorIds.includes(a.id));
    };
    ...
    const getRoomMembers = (room: ChatRoom) => {
        ...
        // 普通模式：总监 + 智囊团成员
        const members = advisors.filter(a => room.advisorIds.includes(a.id));
        // 总监始终在最前面
        return [
    ...
    // 渲染头像（支持emoji和URL图片）
    const renderAvatar = (avatar: string, size: 'sm' | 'md' | 'lg' = 'sm', className?: string) => {
        const sizeClasses = {
                                ...
                                <div key={msg.id} className={clsx("flex gap-3", msg.role === 'user' ? "justify-end" : "")}>
                                    {(msg.role === 'advisor' || msg.role === 'director') && (
                                        <div className={clsx(
                                            ...
                                            {msg.advisorAvatar?.startsWith('http') ? (
                                                <img src={msg.advisorAvatar} alt="" className="w-full h-full object-cover" />
                                            ) : (
                                                <span className="text-sm">{msg.advisorAvatar}</span>
                                            )}
                                        </div>
                                    )}
                                    <div className={clsx(
...
function ManageRoomModal({
    ...
    const [selectedIds, setSelectedIds] = useState<string[]>([...room.advisorIds]);
    ...
    // 渲染头像
    const renderMemberAvatar = (avatar: string, className?: string) => {
        if (avatar.startsWith('http')) {
            return <img src={avatar} alt="" className={clsx("w-full h-full object-cover", className)} />;
        }
        return <span className={clsx("text-lg", className)}>{avatar}</span>;
    };
